...
namespace Arm092\YoutubeUploader;
...
class Youtube {
	 ...
	 *
	 * @var string $OAUTH2_CLIENT_SECRET
	 */
	 ...
	 *
	 * @var string $video_privacy
	 */
	private $video_privacy;

	/**
	 * Uploaded video id
	 *
	 * @var string $uploaded_video_id
	 */
	 ...
	 * Created playlist id
	 *
	 * @var string $created_playlist_id
	 */
	 ...
	 * Session Token
	 *
	 * @var tokenSessionKey $tokenSessionKey
	 */
	...
	public function upload( $video, $params = [] ) {
		...
		if (is_array($params)) {
			...
			$this->video_privacy     = array_key_exists('video_privacy', $params) ? $params['video_privacy'] : '';
		} else {
		...
		if ($client->getAccessToken()) {
			try {
				...
				$status->privacyStatus = $this->video_privacy;
